...
public class NetUtils extends Handler {
    ...
    public void init(Handler handler) {
        Log.i(TAG, "init client send broadcast our ip and get tv's ip");
        mHandler = handler;
...
//            Log.d(TAG, "wait 2000ms and send message:" + msgWhat + " again. (UUID:" + UUID + ")");
//            sendEmptyMessageDelayed(msgWhat, 1500);
    ...
    public void sendMsg(String msg) {
        ...
        try {
            Log.d(TAG, "send msg is " + new String(data, DATA_SEGMENT_START_INDEX, length, "utf-8"));
    ...
    private void parseReceiveBuffer(byte[] buffer, DatagramPacket datagramPacket) throws UnsupportedEncodingException {
        ...
        Log.d(TAG, "parseReceiveBuffer::version[" + version + "] deviceId[" + deviceId
        ...
        if (dataMap.size() != 0) {
            if (dataMap.containsKey(UUID) && deviceId == DEVICE_TYPE_PHONE) {
                ...
                Log.d(TAG, "get suspendBuffer: " + UUID + "/" + type + " and remove it");
                if (type != NetConst.STTP_LOAD_TYPE_REQUEST_CONNECTSTATUS) {

                    int msgWhat = WHAT_RESPONSE_STATE | (UUID << 8);
                    if (hasMessages(msgWhat)) {
                        Log.i(TAG, "remove suspend message: " + msgWhat);
                    ...
                    if (stateMap.containsKey(ip)) {
                        stateMap.put(ip, 0);
                        Log.i(TAG, "reset state counter to 0" + " IP " + ip);
                    }
                }
                dataMap.remove(UUID);
        ...
        if (load_type == NetConst.STTP_LOAD_TYPE_REQUEST_CONNECTION
            ...
            if (ipList != null && !ipList.contains(ip)) {
                ...
                Log.i(TAG, "REVEIVE CONNECTION FROM " + deviceName + "[" + ip + "]");
                if (ipClient == null) {
                    ipClient = ip;
                }
                ipList.add(ip);
    ...
    public void handleMessage(Message msg) {
        ...
        if (what == WHAT_RESPONSE_STATE) {
            ...
            if (dataMap.containsKey(randomKey)) {
                ...
                mPool.submit(SendRunnable);
                Log.i(TAG, "resend message: " + randomKey);
            }
        } else if (what == WHAT_CHECKOUT_DEVICE_STATUS_REMOTE) {
            if (ipList.size() > 0) {
                Log.i(TAG, "WHAT_CHECKOUT_DEVICE_STATUS_REMOTE");
            ...
            Log.i(TAG, "WHAT_CHECKOUT_DEVICE_STATUS_LOCAL");
            checkDeviceStatus();
        }
    }
    ...
    private void checkDeviceStatus() {
        ...
        while (iter.hasNext()) {
            ...
            if (load_type == NetConst.STTP_LOAD_TYPE_REQUEST_CONNECTSTATUS) {
                ...
                counter++;
                Log.i(TAG, "UUID " + key + " counter " + counter + " IP " + info.dstIp);
                if (counter > 5) {
                    Log.d(TAG, "checkDeviceStatus ip: " + info.dstIp + " connect time out");
    ...
    public class InitGetClient implements Runnable {
        ...
        public void run() {
            try {
                ...
                Log.i(TAG, "########ipClient is " + ipClient + "########");
            } catch (IOException e) {
                Log.e(TAG, getClass() + ":" + e.toString());
                e.printStackTrace();
            } catch (InterruptedException e) {
                e.printStackTrace();
            } finally {
                Log.w(TAG, "close InitGetClient socket");
                initClientSocket.close();
            }
        }
    }
    ...
    public class ReceiveRunnale implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while (isFlag) {
                    Log.d(TAG, "------------>enter loop and wait receive data<----------");
                    receiveSocket.receive(datagramPacket);
                    Log.i(TAG, "receive from: " + datagramPacket.getAddress().getHostAddress());
                ...
                if (!receiveSocket.isClosed()) {
                    Log.w(TAG, "close ReceiveRunnale socket");
                    receiveSocket.close();
                    receiveSocket = null;
                }
            }
    ...
    public class SendRunnable implements Runnable {
        ...
        public void run() {
            try {
                ...
                Log.i(TAG, "send message: " + (buffer[4] & 0xFF) + " to: " + dstIp);
            } catch (SocketException e) {
                Log.e(TAG, getClass() + ":" + e.toString());
                e.printStackTrace();
            } catch (IOException e) {
